...
final class CardFile {
    ...
    private TreeMap<UUID, Pet> pets = new TreeMap<>();
...
//        UUID id = UUID.randomUUID();
     ...
     * @return значение ID добавленной карты питомца.
     */
    public UUID setPet(String nickname, float weight, Person owner) throws Exception {
     ...
     */
    public TreeMap<UUID, Pet> findPet(String nickname){
        TreeMap<UUID, Pet> pt = new TreeMap<>();
        if (pets.isEmpty()) return pt;
        Map<UUID, Pet> map = pets.entrySet().stream()
    ...
    public String toString() {
        Iterator<Map.Entry<UUID, Pet>> iterator = pets.entrySet().iterator();
